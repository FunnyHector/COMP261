...
public class TextProcessor {
    ...
    // the list of algorithms in the drop-down box. if you wanted to add
    // another, you would include
    // it here and add another case in the onAlgorithmRun method.
    ...
    private void initialise() {
        ...
        // first, setup a listener to call onEditorLoad when loading a file.
        ...
        searchField.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (index == -1) {
                    ...
                    if (textEditor.requestFocusInWindow()) {
                        textEditor.setSelectionStart(index);
                        textEditor.setSelectionEnd(index + pattern.length());
                        textEditor.setSelectionColor(Color.YELLOW);
                    } else {
                        JOptionPane.showMessageDialog(frame,
                                "Failed to request focus. (But the pattern is found)");
                    }
                }
            }
        });
        ...
        load.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ev) {
                ...
                if (fileChooser.showOpenDialog(frame) == JFileChooser.APPROVE_OPTION) {
                    ...
                    compressionFileSelection.setText("<html>LZ77 Window Size: " + LempelZiv.getWindowSize()
                            + "<br>input: " + inputName + "<br>output: " + outputName + "</html>");
                }
            }
        });
        ...
        // a text field to set Lempel-Ziv search window size
        JTextField windowSize;
        windowSize = new JTextField(10);
        windowSize.setMaximumSize(new Dimension(0, 25));
        windowSize.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                try {
                    int size = Integer.valueOf(windowSize.getText());
                    LempelZiv.setWindowSize(size);
        ...
        // leftTop.add(AlgorithmList, BorderLayout.SOUTH);

        // put it all together.
        JPanel leftTop = new JPanel();
        GridLayout gridLayout = new GridLayout(5, 1);
        gridLayout.setVgap(LAYOUT_GAP);
        leftTop.setLayout(gridLayout);
        leftTop.add(load);
        leftTop.add(save);
        leftTop.add(AlgorithmList);
        leftTop.add(new JLabel("LZ77 window size:"));
        leftTop.add(windowSize);
        ...
        compressionFileSelection = new JLabel(
                "<html>LZ77 Window Size: " + LempelZiv.getWindowSize() + "<br>input:<br>output:</html>");
        ...
        // the left panel is just leftTop, a vertical space, and the run button
        // at the bottom.
        ...
        JScrollPane scroll2 = new JScrollPane(compressionOutput);
        ...
        right.add(scroll2, BorderLayout.CENTER);
            ...
            public boolean dispatchKeyEvent(KeyEvent ev) {
                if (ev.getID() == KeyEvent.KEY_PRESSED) {
                    ...
                    if (shortcut >= 0) { // for marking?
                        ...
                        String inputName = compressionInputFile == null ? "" : compressionInputFile.getName();
                        String outputName = compressionOutputFile == null ? ""
                                : compressionOutputFile.getName();
                        compressionFileSelection.setText(
                                "<html>LZ77 Window Size: " + LempelZiv.getWindowSize() + "<br>input: "
                                        + inputName + "<br>output: " + outputName + "</html>");
                    }
                }
                return false;
            }
        ...
        frame = new JFrame("TextProcessor");
    ...
    /**
     * Handles all the logic of setting up and running each algorithm, such as making input and
     * output streams, and writing some useful information to the output area in the GUI.
     */
    private void onAlgorithmRun() {
        try {
            if (compressionInputFile == null || !compressionInputFile.exists()) {
                JOptionPane.showMessageDialog(frame, "Input file does not exist or is unselected.");
                return;
            }
            if (compressionOutputFile == null) {
                JOptionPane.showMessageDialog(frame, "Output file is unselected.");
                return;
            }
            if (!compressionInputFile.getName().endsWith(".txt")) {
                JOptionPane.showMessageDialog(frame,
                        "This assignment only deals with compressing .txt files.");
            ...
            if (algorithm.equals("Huffman coding")) {
                ...
                // figure out file sizes. the encoded string represents bits, so
                // we divide it by 8 to get the size in bytes.
                ...
                compressionOutput.append(
                        "original and decoded texts " + (text.equals(decoded) ? "" : "DO NOT ") + "match.\n");
                ...
                // check they're the same and display user output.
                compressionOutput.append("\noriginal and decoded texts "
                        + (text.equals(decompressed) ? "" : "DO NOT ") + "match!\n");
