...
public abstract class GUI {
    ...
    private boolean isSelectingStartNode = false;
    private boolean isSelectingEndNode = false;
    ...
    /**
     * Is called when the mouse is pressed, and is passed the MouseEvent object
     * for that press.
     */
    protected abstract void onPressed(MouseEvent e);

    /**
     * Is called when the mouse is clicked released, and is passed the
     * MouseEvent object for that click.
     */
     ...
     * Specifies six moves in enum class.
     */
    ...
    /**
     * Switch between distance-first and time-first navigation.
     */
    protected abstract void switchTimeDistance();
    ...
    /**
     * This method returns whether the programme is selecting start
     * intersection for navigation.
     * @return  true if it is selecting start intersection, false if not.
     */
    public boolean isSelectingStartNode(){
        return isSelectingStartNode;
    }
    
    /**
     * This method returns whether the programme is selecting end
     * intersection for navigation.
     * @return  true if it is selecting end intersection, false if not.
     */
    public boolean isSelectingEndNode(){
        return isSelectingEndNode;
    }
    ...
    // drop down list, to give hint
    // private JComboBox searchPopDown;
    ...
    private void initialise() {
        ...
        //============New buttons for Assignment 2===========
        JToggleButton selectStartNode = new JToggleButton ("Start");
        selectStartNode.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ev) {
                if (!isSelectingEndNode) {
                    isSelectingStartNode = selectStartNode.isSelected();
                } else {
                    selectStartNode.setSelected(false);
                }
            }
        });
        
        JToggleButton selectEndNode = new JToggleButton ("End");
        selectEndNode.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ev) {
                if (!isSelectingStartNode) {
                    isSelectingEndNode = selectEndNode.isSelected();
                } else {
                    selectEndNode.setSelected(false);
                }
            }
        });
        
        JButton articulPoint = new JButton ("Articulation");
        articulPoint.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ev) {
        ...
        JButton speedOrDistance = new JButton ("Distance \u2191");
        speedOrDistance.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ev) {
                if (speedOrDistance.getText().equals("Distance \u2191")) {
                    speedOrDistance.setText("Time \u2191");
                } else {
                    speedOrDistance.setText("Distance \u2191");
                }
                switchTimeDistance();
                redraw();
            }
        ...
        // exactly what you expect. three common LayoutManagers are the
        // BoxLayout,
        ...
        navigation.setMaximumSize(new Dimension(500, 60));
        navigation.setLayout(new GridLayout(2, 5));
        navigation.add(out);
        navigation.add(north);
        navigation.add(in);
        navigation.add(selectStartNode);
        navigation.add(articulPoint);
        navigation.add(west);
        navigation.add(south);
        navigation.add(east);
        navigation.add(selectEndNode);
        navigation.add(speedOrDistance);;
