...
public class Robot {
    ...
    public void turnLeft() {
        debug("turnL");
        currentAction = Action.TURN_LEFT;
        switch (dir) {
        case NORTH:
    ...
    public void turnRight() {
        debug("turnR");
        currentAction = Action.TURN_RIGHT;
        switch (dir) {
        case NORTH:
            targetDir = Direction.EAST;
            break;
    ...
    public void turnAround() {
        debug("turnAround");
        currentAction = Action.TURN_AROUND;
        switch (dir) {
        case NORTH:
            targetDir = Direction.SOUTH;
            break;
    ...
    public void takeFuel() {
        debug("takeFuel");
        currentAction = Action.TAKE_FUEL;
        processFuelAndBlock();
        readState();
    }
    ...
    public enum Action {
        MOVE, TURN_LEFT, TURN_RIGHT, TURN_AROUND, TAKE_FUEL, WAIT
    }
    ...
    private static final int FUEL_IDLE = 3, FUEL_MOVE = 6, FUEL_TURN = 5, FUEL_SHIELD = 15;
    ...
    public void draw(Graphics2D g2d, double time) {
        ...
        if (((int) (time * 10)) % 2 == 0 && currentAction != Action.TAKE_FUEL && currentAction != Action.WAIT) {
    ...
    private Point toRelative(Point p) {
        ...
        switch (dir) {
        case NORTH:
            point = new Point(rx - x, y - ry);
            break;
        case SOUTH:
            point = new Point(x - rx, ry - y);
            break;
        case WEST:
            point = new Point(y - ry, x - rx);
            break;
        case EAST:
        default:
            point = new Point(ry - y, rx - x);
            break;
        }
        return point;
    }
